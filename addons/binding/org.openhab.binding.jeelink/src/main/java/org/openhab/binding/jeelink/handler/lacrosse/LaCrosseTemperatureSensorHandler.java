...
public class LaCrosseTemperatureSensorHandler extends JeeLinkSensorHandler<LaCrosseTemperatureReading> {
    ...
    public void updateReadingStates(LaCrosseTemperatureReading reading) {
        if (reading != null) {
            ...
            // Sensor Type has been added lately, so existing sensors might not yet have it.
            Channel sensorTypeChannel = getThing().getChannel(SENSOR_TYPE_CHANNEL);
            if (sensorTypeChannel != null) {
                updateState(sensorTypeChannel.getUID(), new DecimalType(reading.getSensorType()));
            }
        }
    }
