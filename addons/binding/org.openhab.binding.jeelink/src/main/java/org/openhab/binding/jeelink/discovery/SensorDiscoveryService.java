            ...
            if (capture.get()) {
                ...
                for (Thing t : existingThings) {
                    sensorThingExists = sensorThingExists
                            || id.equals(t.getConfiguration().getProperties().get(PROPERTY_SENSOR_ID));
                }
                ...
                if (!sensorThingExists) {
                    ...
                    DiscoveryResult discoveryResult = DiscoveryResultBuilder.create(sensorThing)
                            .withLabel(def.getName() + " (ID = " + id + ")").withBridge(bridge.getThing().getUID())
                            .withProperty(PROPERTY_SENSOR_ID, id).withProperty(PROPERTY_UPDATE_INTERVAL, 60).build();
